...
const addDiscount2 = async (params, rechargeResult) => {
  try {
    var userAccount = await db.Account.findById({ _id: params.userId });
...
const updateReferalUserDiscount = async (
  ...
  try {
    var userAccount = await db.Account.findById({
      _id: referredUser,
    });
...
const rechargeFunction = async (params, operator, filteredOperatorConfig) => {
  try {
    ...
    let rechargeData = await doRecharge(filteredOperatorConfig, payload);
    // rechargeData.rechargeOperator = operator;
    rechargeData.operatorConfig = filteredOperatorConfig;
...
const doRecharge = async (filterOperatorConfig, payload) => {
  try {
    ...
    if (apiName == "RechargeWale") {
      ...
      // rechargeWaleRes.rechargeApi = filterOperatorConfig;
      return rechargeWaleRes;
    }

    if (apiName == "Ambika") {
      let ambikaRes = await ambikaRecharge(payload);
      // ambikaRes.rechargeApi = filterOperatorConfig;
      return ambikaRes;
    }
  } catch (err) {
    console.error(err);
